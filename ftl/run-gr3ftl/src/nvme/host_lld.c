...
// 从硬件命令队列中取出一条 NVMe 命令 的函数，相当于“固件从 NVMe 硬件接口抓取主机下发命令”的入口。
/*命令来自的队列 ID（Submission Queue ID）。

cmdSlotTag：命令槽标识（固件内部区分不同命令）。

cmdSeqNum：命令序列号。

cmdDword：指向一个数组（16 个 DWORD = 64 字节），存放 NVMe 命令本体。*/
unsigned int get_nvme_cmd(unsigned short *qID, unsigned short *cmdSlotTag, unsigned int *cmdSeqNum, unsigned int *cmdDword)
{
	...
	/*
	NVME_CMD_FIFO_REG_ADDR 是 NVMe 命令 FIFO（硬件队列）的地址。

	nvmeReg.dword 读出来包含了：
	...
	*/
	...
	if(nvmeReg.cmdValid == 1)//如果 cmdValid = 1，说明 FIFO 里有新命令。
	{
		unsigned int addr;
		unsigned int idx;
		*qID = nvmeReg.qID;
...
void set_direct_tx_dma(unsigned int devAddr, unsigned int pcieAddrH, unsigned int pcieAddrL, unsigned int len)
{
	...
	//xil_printf("set_direct_tx_dma done\n");
}
