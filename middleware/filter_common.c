		...
		int *hit_count_out)//blk_dat_offset 可以删除
{
	...
	for(int i=0; i<page_tuple_cnt; i++)
	{
		...
		if(item->lp_flags != LP_NORMAL)
		{
            ...
            printf("Non-normal tuple flag = %d\r\n", item->lp_flags);
			continue;
		}
...
void ISSUE_FILTER(tupdesc_t *csd_tupdesc, filter_t *csd_filter,qual_t *csd_qual,unsigned char buf[8192])
{
	uint8_t *base = (uint8_t *)(buf);  // 拿到当前页起始地址的指针
	...
	int filter_cnt = csd_qual->file_cnt;
	unsigned char *page_data = (unsigned char*) buf;
	...
	DBG_INFO("get_tuple_attr_pos start\n");
	...
	for(int i = 0; i < hit_count_out; i ++)
	{
		...
		if (append_tuple_to_mem_page(&writer, &tuple) < 0)
		{
			...
			printf("Failed to append tuple %d to mem page\r\n", index);
			break;
		}
	}
	...
	memcpy(result_buf,&result_page_num,sizeof(result_page_num));
