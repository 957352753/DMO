...
{
	volatile unsigned int* ctrl0 =
		(volatile unsigned int *)(uintptr_t)XPAR_IODELAY_IF_0_DQS_BASEADDR;
	volatile iodelay_if* data0 = (volatile iodelay_if *)(uintptr_t)
		(XPAR_IODELAY_IF_0_DQS_BASEADDR + 0x8000U);
...
{
	volatile unsigned int* ctrl0 =
		(volatile unsigned int *)(uintptr_t)XPAR_IODELAY_IF_0_BASEADDR;
	volatile iodelay_if* data0 = (volatile iodelay_if *)(uintptr_t)
		(XPAR_IODELAY_IF_0_BASEADDR + 0x8000U);
	int i;
...
{
	uintptr_t base = (uintptr_t)t4nscRegisterBaseAddress;

	t4regs->t4regID = (T4REG_ID *)(base + 0x0000U);
	t4regs->t4regCFG = (T4REG_CFG *)(base + 0x1000U);
	t4regs->t4regEXT = (T4REG_EXT *)(base + 0x2000U);
	t4regs->t4regCC = (T4REG_CC *)(base + 0x3000U);
	t4regs->t4regBP = (T4REG_BP *)(base + 0x3800U);
	t4regs->t4regSP = (T4REG_SP *)(base + 0x4000U);
}
...
void V2FGetReadyBusy(T4REGS* t4regs, volatile unsigned int* report)
{
	T4REG_CMD_GET_READYBUSY getReadyBusyCmd;

	getReadyBusyCmd.cmdSelect = T4NSC_CMD_GET_READYBUSY;
	getReadyBusyCmd.reserved = 0;
	getReadyBusyCmd.reportAddress = (unsigned int)(uintptr_t)report;
...
void __attribute__((optimize("O0"))) V2FSetFeaturesSync(T4REGS* t4regs, int way, unsigned int feature0x02, unsigned int feature0x10, unsigned int feature0x91, unsigned int feature0x01, unsigned int payLoadAddr)
{
	volatile unsigned int* payload =
		(volatile unsigned int *)(uintptr_t)payLoadAddr;
	volatile unsigned int* status =
		(volatile unsigned int *)(uintptr_t)(payLoadAddr + 4U);
...
static inline __attribute__((always_inline)) void V2FArmDmaCompletion(unsigned int* completion)
{
	/* Publish the cleared flag before ringing the controller doorbell. */
...
void __attribute__((optimize("O0"))) V2FReadPageTransferAsync(T4REGS* t4regs, int way, void* pageDataBuffer, void* spareDataBuffer, unsigned int* errorInformation, unsigned int* completion, unsigned int rowAddress)
{
	...
	readpagepSLC.pageDataAddress = (unsigned int)(uintptr_t)pageDataBuffer;
	readpagepSLC.spareDataAddress = (unsigned int)(uintptr_t)spareDataBuffer;
	readpagepSLC.errorInfoAddress = (unsigned int)(uintptr_t)errorInformation;
	readpagepSLC.completionReportAddress = (unsigned int)(uintptr_t)completion;

	while (V2FIsControllerBusy(t4regs));
	V2FArmDmaCompletion(completion);
...
void __attribute__((optimize("O0"))) V2FReadPageTransferRawAsync(T4REGS* t4regs, int way, void* pageDataBuffer, unsigned int* completion)
{
	...
	readPageTransferRaw.pageDataAddress = (unsigned int)(uintptr_t)pageDataBuffer;
	readPageTransferRaw.completionReportAddress = (unsigned int)(uintptr_t)completion;
...
void __attribute__((optimize("O0"))) V2FProgramPageAsync(T4REGS* t4regs, int way, unsigned int rowAddress, void* pageDataBuffer, void* spareDataBuffer)
{
	...
	progPagepSLC.pageDataAddress = (unsigned int)(uintptr_t)pageDataBuffer;
	progPagepSLC.spareDataAddress = (unsigned int)(uintptr_t)spareDataBuffer;
...
void __attribute__((optimize("O0"))) V2FStatusCheckAsync(T4REGS* t4regs, int way, unsigned int* statusReport)
{
	...
	readStatusCmd.reportAddress = (unsigned int)(uintptr_t)statusReport;
	*statusReport = 0;
