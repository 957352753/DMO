...
#ifndef FILTER_QUEUE_TRACE_ENABLE
#define FILTER_QUEUE_TRACE_ENABLE 0
#endif

#if FILTER_QUEUE_TRACE_ENABLE
#define FILTER_QUEUE_TRACE(...) xil_printf(__VA_ARGS__)
#else
#define FILTER_QUEUE_TRACE(...) do { } while(0)
#endif
...
unsigned int GetFromSliceReqQ()
{
	...
	if(reqPoolPtr->reqPool[reqSlotTag].nextReq != REQ_SLOT_TAG_NONE)// 有多个请求
	{
		sliceReqQ.headReq = reqPoolPtr->reqPool[reqSlotTag].nextReq;// 队首的下一个请求存在 → 把它设为新的队首
		reqPoolPtr->reqPool[reqPoolPtr->reqPool[reqSlotTag].nextReq].prevReq = REQ_SLOT_TAG_NONE;//同时把新队首的 prevReq 置为 NONE（因为它现在是第一个请求）
	}
	else//队列中只有这一个请求。
	{
		sliceReqQ.headReq = REQ_SLOT_TAG_NONE;
...
void GetFromFilterReqQ(unsigned int reqSlotTag)
{
	FILTER_QUEUE_TRACE("GetFromFilterReqQ: %d\r\n",reqSlotTag);
	unsigned int prevReq, nextReq;
	// 从请求池中取出要删除请求的 前一个请求 和 后一个请求。
	prevReq = reqPoolPtr->reqPool[reqSlotTag].prevReq;
	nextReq = reqPoolPtr->reqPool[reqSlotTag].nextReq;

	if((nextReq != REQ_SLOT_TAG_NONE) && (prevReq != REQ_SLOT_TAG_NONE))//调整两边节点的链接，使它们互相指向，跳过当前节点。
	{
		reqPoolPtr->reqPool[prevReq].nextReq = nextReq;
		reqPoolPtr->reqPool[nextReq].prevReq = prevReq;
	}
	else if((nextReq == REQ_SLOT_TAG_NONE) && (prevReq != REQ_SLOT_TAG_NONE))//修改前一个节点的 next 为 NONE，并更新队列尾为 prevReq。
	{
		reqPoolPtr->reqPool[prevReq].nextReq = REQ_SLOT_TAG_NONE;
	}
	else if((nextReq != REQ_SLOT_TAG_NONE) && (prevReq == REQ_SLOT_TAG_NONE))//修改下一个节点的 prev 为 NONE，并更新队列头为 nextReq。
	{
		reqPoolPtr->reqPool[nextReq].prevReq = REQ_SLOT_TAG_NONE;
	}
...
void CompleteFilterReq(unsigned int reqSlotTag)
{
	if(reqSlotTag == REQ_SLOT_TAG_NONE)
		return;

	FILTER_QUEUE_TRACE("CompleteFilterReq: %d\r\n", reqSlotTag);
	reqPoolPtr->reqPool[reqSlotTag].reqQueueType = REQ_QUEUE_TYPE_NONE;
	reqPoolPtr->reqPool[reqSlotTag].prevReq = REQ_SLOT_TAG_NONE;
	reqPoolPtr->reqPool[reqSlotTag].nextReq = REQ_SLOT_TAG_NONE;
...
void PutToNandReqQ(unsigned int reqSlotTag, unsigned chNo, unsigned wayNo)
{
	...
	if(FILTER_TAG == 1)
		FilterProfileStartNandRead(reqSlotTag);
}
...
void GetFromNandReqQ(unsigned int chNo, unsigned int wayNo, unsigned int reqStatus, unsigned int reqCode)
{
	...
	notCompletedNandReqCnt--;
	if(FILTER_TAG == 1)
		FilterProfileCompleteNandRead(reqSlotTag);
