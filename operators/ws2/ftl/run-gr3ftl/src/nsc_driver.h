...
{                                                                                   \
    volatile unsigned int *dest_ptr = (volatile unsigned int *)((t4regs)->t4regSP); \
    unsigned int i;                                                                 \
    unsigned int *src_ptr = (unsigned int *)&(cmdpayload);                          \
                                                                                    \
    for(i = 0; i < sizeof(cmdtype) / sizeof(int); i++)                              \
    {                                                                               \
        *dest_ptr++ = *src_ptr++;                                                   \
    }                                                                               \
}
...
void V2FSetScramblerState(T4REGS* t4regs, int enable);
void V2FResetSync(T4REGS* t4regs, int way);
void V2FSetFeaturesSync(T4REGS* t4regs, int way, unsigned int feature0x02, unsigned int feature0x10, unsigned int feature0x91, unsigned int feature0x01, unsigned int payLoadAddr);
