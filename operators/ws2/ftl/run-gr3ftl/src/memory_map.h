...
//FILTER ADDR
#define FILTER_START_ADDR               0x00300000

#define FILTER_INFO_DRAM_DATA_BUFFER    0x00300000
#define FILTER_RESULT_DRAM_DATA_BUFFER  0x00304000

#if FILTER_RESULT_MAX_PAGES <= 16
#define FILTER_HIT_DRAM_DATA_BUFFER     0x00324000
#define FILTER_PROFILE_DRAM_DATA_BUFFER 0x00334000
#define FILTER_BATCH_DRAM_DATA_BUFFER   0x00344000
#else
#define FILTER_HIT_DRAM_DATA_BUFFER     0x00344000
#define FILTER_PROFILE_DRAM_DATA_BUFFER 0x00364000
#define FILTER_BATCH_DRAM_DATA_BUFFER   0x00368000
#endif

#define FILTER_ACCEL_BASE_ADDR          0xA0050000
#define FILTER_END_ADDR                 0x0FFFFFFF

#define FILTER_PROJECTION_DRAM_DATA_BUFFER \
    (FILTER_BATCH_DRAM_DATA_BUFFER + FILTER_ACCEL_MAX_BATCH_PAGES * 8192U)
#define FILTER_PROJECTION_DRAM_DATA_BYTES \
    (FILTER_ACCEL_MAX_BATCH_PAGES * FILTER_PROJECTION_STAGING_BYTES_PER_PAGE)

/* Projection, FPGA aggregate partials, and FPGA join hashes are mutually
 * exclusive and intentionally share the same per-page staging region. */
#define FILTER_COMPUTE_DRAM_DATA_BUFFER FILTER_PROJECTION_DRAM_DATA_BUFFER
#define FILTER_COMPUTE_DRAM_DATA_BYTES  FILTER_PROJECTION_DRAM_DATA_BYTES

#define FILTER_COMPUTE_WORK_DRAM_DATA_BUFFER \
    (FILTER_PROJECTION_DRAM_DATA_BUFFER + FILTER_PROJECTION_DRAM_DATA_BYTES)
#define FILTER_JOIN_WORK_DRAM_DATA_BUFFER \
    (FILTER_COMPUTE_WORK_DRAM_DATA_BUFFER + FILTER_COMPUTE_WORK_BYTES)

#if (FILTER_PROJECTION_DRAM_DATA_BUFFER + FILTER_PROJECTION_DRAM_DATA_BYTES) > FILTER_END_ADDR
#error "filter projection staging exceeds the reserved filter DRAM region"
#endif

#if (FILTER_JOIN_WORK_DRAM_DATA_BUFFER + FILTER_JOIN_WORK_BYTES) > FILTER_END_ADDR
#error "filter compute/join workspaces exceed the reserved filter DRAM region"
#endif
