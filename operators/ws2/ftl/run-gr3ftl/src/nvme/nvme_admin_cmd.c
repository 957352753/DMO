...
static void set_admin_cpl_error(NVME_COMPLETION *nvmeCPL, unsigned int sct, unsigned int sc)
{
	nvmeCPL->statusFieldWord = 0;
	nvmeCPL->specific = 0x0;
...
void handle_set_features(NVME_ADMIN_COMMAND *nvmeAdminCmd, NVME_COMPLETION *nvmeCPL)
{
	...
	switch(features.FID)
	{
		...
		default:
		{
			...
			set_admin_cpl_error(nvmeCPL, SCT_GENERIC_COMMAND_STATUS, SC_INVALID_FIELD_IN_COMMAND);
			break;
		}
	}
...
void handle_identify(NVME_ADMIN_COMMAND *nvmeAdminCmd, NVME_COMPLETION *nvmeCPL)
{
	...
	{
		unsigned int *namespaceIdList = (unsigned int *)pIdentifyData;

		if((nvmeAdminCmd->PRP1[0] & 0x3) != 0 || (nvmeAdminCmd->PRP2[0] & 0x3) != 0)
			xil_printf("NS List: PRP1 = %08X_%08X, PRP2 = %08X_%08X\r\n", nvmeAdminCmd->PRP1[1], nvmeAdminCmd->PRP1[0], nvmeAdminCmd->PRP2[1], nvmeAdminCmd->PRP2[0]);
	...
	{
		if((nvmeAdminCmd->PRP1[0] & 0x3) != 0 || (nvmeAdminCmd->PRP2[0] & 0x3) != 0)
			xil_printf("NS Desc: PRP1 = %08X_%08X, PRP2 = %08X_%08X\r\n", nvmeAdminCmd->PRP1[1], nvmeAdminCmd->PRP1[0], nvmeAdminCmd->PRP2[1], nvmeAdminCmd->PRP2[0]);
	...
	{
		xil_printf("Unsupported Identify CNS: 0x%X, NSID: 0x%X, DW10: 0x%08X, DW11: 0x%08X\r\n",
			identifyInfo.CNS, nvmeAdminCmd->NSID, nvmeAdminCmd->dword10, nvmeAdminCmd->dword11);
...
void handle_nvme_admin_cmd(NVME_COMMAND *nvmeCmd)
{
	...
	switch(opc)
	{
		...
		{
			set_admin_cpl_error(&nvmeCPL, SCT_GENERIC_COMMAND_STATUS, SC_INVALID_COMMAND_OPCODE);
			break;
		}
