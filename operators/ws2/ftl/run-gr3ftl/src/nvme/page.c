...
#ifndef FILTER_PAGE_TRACE_ENABLE
#define FILTER_PAGE_TRACE_ENABLE 0
#endif

#if FILTER_PAGE_TRACE_ENABLE
#define FILTER_PAGE_TRACE(...) xil_printf(__VA_ARGS__)
#else
#define FILTER_PAGE_TRACE(...) do { } while(0)
#endif
...
int switch_to_next_page(MemPageWriter *w)
{
    ...
    {
        FILTER_PAGE_TRACE("No more space: all %d pages full", MAX_PAGES);
        return -1;
    }
...
int append_tuple_to_mem_page(MemPageWriter *w, HeapTuple tuple) // HeapTupleData 只使用到 t_len 和 t_data
{
    ...
    {
        FILTER_PAGE_TRACE("Current page %d full, need to switch page\r\n", w->cur_page_index);
        FILTER_PAGE_TRACE("Total tuples in current page: %d\r\n", w->tuple_count);
        // 当前页放不下了，切换新页
        if (switch_to_next_page(w) < 0)
        {
            FILTER_PAGE_TRACE("Failed to switch to next page\r\n");
            return -1 ;  // 切换页失败
        }
