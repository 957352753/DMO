...
void InitReqScheduler()
{
	...
	completeFlagTablePtr =
		(P_COMPLETE_FLAG_TABLE)(uintptr_t)COMPLETE_FLAG_TABLE_ADDR;
	statusReportTablePtr =
		(P_STATUS_REPORT_TABLE)(uintptr_t)STATUS_REPORT_TABLE_ADDR;
	eccErrorInfoTablePtr =
		(P_ERROR_INFO_TABLE)(uintptr_t)ERROR_INFO_TABLE_ADDR;
	retryLimitTablePtr =
		(P_RETRY_LIMIT_TABLE)(uintptr_t)RETRY_LIMIT_TABLE_ADDR;

	dieStateTablePtr = (P_DIE_STATE_TABLE)(uintptr_t)DIE_STATE_TABLE_ADDR;
	wayPriorityTablePtr =
		(P_WAY_PRIORITY_TABLE)(uintptr_t)WAY_PRIORITY_TABLE_ADDR;
...
{
	//xil_printf("SyncAllLowLevelReqDone\r\n");
...
int SyncAllLowLevelReqDoneTimeout(unsigned int timeoutMs)
{
	XTime start;
	XTime now;
	uint64_t timeoutCounts;

	if(timeoutMs == 0U)
		return -1;

	timeoutCounts = ((uint64_t)COUNTS_PER_SECOND * timeoutMs + 999U) / 1000U;
	XTime_GetTime(&start);
		  ...
		  notCompletedNandReqCnt || blockedReqCnt)
	{
		...
		SchedulingNandReq();
		XTime_GetTime(&now);
		if((uint64_t)(now - start) >= timeoutCounts)
			return -1;
	}
...
{
	//xil_printf("SyncReleaseEraseReq\r\n");
...
{
	//xil_printf("SchedulingNandReqPerCh\r\n");
...
void IssueNandReq(unsigned int chNo, unsigned int wayNo)
{
	...
	//xil_printf("IssueNandReq\r\n");
	reqSlotTag  = nandReqQ[chNo][wayNo].headReq;
	rowAddr = GenerateNandRowAddr(reqSlotTag);
	dataBufAddr = (void *)(uintptr_t)GenerateDataBufAddr(reqSlotTag);
	spareDataBufAddr = (void *)(uintptr_t)GenerateSpareDataBufAddr(reqSlotTag);
	...
	// if(FILTER_TAG && reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_READ_TRANSFER)//已经从Nand 里面读取数据
	// {
	// 	//过滤处理逻辑
	// 	xil_printf("buffer not hit\r\n");
	...
	// if(ADD_ONE_TAG1)
	// {
	// 	xil_printf("reqPoolPtr->reqPool[reqSlotTag].reqCode: %d\r\n",reqPoolPtr->reqPool[reqSlotTag].reqCode);
	// 	unsigned int *devAddr = (unsigned int *)dataBufAddr;
...
unsigned int GenerateDataBufAddr(unsigned int reqSlotTag)
{
	...
	else if(reqPoolPtr->reqPool[reqSlotTag].reqType == REQ_TYPE_NVME_DMA || reqPoolPtr->reqPool[reqSlotTag].reqType == REQ_TYPE_NVME_FILTER)
	{
		if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.dataBufFormat == REQ_OPT_DATA_BUF_ENTRY)
...
unsigned int CheckReqStatus(unsigned int chNo, unsigned int wayNo)
{
	...
	volatile unsigned int* completeFlagPtr;

	reqSlotTag = nandReqQ[chNo][wayNo].headReq;
	if(dieStateTablePtr->dieState[chNo][wayNo].reqStatusCheckOpt == REQ_STATUS_CHECK_OPT_COMPLETION_FLAG)
	{
		completeFlagPtr = &completeFlagTablePtr->completeFlag[chNo][wayNo];
		completeFlag = *completeFlagPtr;

		if(V2FTransferComplete(completeFlag))
		{
			/* The NAND controller writes page data before the completion flag.
			 * Acquire those DMA writes before ARM code can consume the buffer. */
...
{
	//xil_printf("ExecuteNandReq\r\n");
	unsigned int reqSlotTag, rowAddr, phyBlockNo;
	unsigned char* badCheck ;

	reqSlotTag = nandReqQ[chNo][wayNo].headReq;
	if(ADD_ONE_TAG1) xil_printf("dieState : %d \r\n",dieStateTablePtr->dieState[chNo][wayNo].dieState);
			...
			else if(reqStatus == REQ_STATUS_FAIL)
			{
				...
				if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.nandEcc == REQ_OPT_NAND_ECC_OFF)
					if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.dataBufFormat == REQ_OPT_DATA_BUF_ADDR)
					{
						...
						badCheck = (unsigned char *)(uintptr_t)
