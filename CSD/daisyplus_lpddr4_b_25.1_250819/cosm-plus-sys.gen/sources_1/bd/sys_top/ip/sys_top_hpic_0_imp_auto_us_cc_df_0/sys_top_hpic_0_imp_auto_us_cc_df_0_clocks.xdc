...
###############################################################################################################
# Core-Level Timing Constraints for axi_dwidth_converter Component "sys_top_hpic_0_imp_auto_us_cc_df_0"
###############################################################################################################
...
create_waiver -type CDC -id {CDC-10} -user "axi_dwidth_converter" -desc "Timing-uncritical signals driving a simple status LED (combined from multiple sources)" -tags "1165309" -scope -internal \
    -to [get_pins -quiet -filter REF_PIN_NAME=~*PRE -of_objects [get_cells -hierarchical -filter {NAME =~ */inst/gen_upsizer.gen_full_upsizer.axi_upsizer_inst/*inst_fifo_gen/*arststages_ff_reg[*]*}]]	
