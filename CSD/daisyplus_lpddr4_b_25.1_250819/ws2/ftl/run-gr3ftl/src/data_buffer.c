...
查找阶段：
根据请求的 logicalSliceAddr（逻辑块地址），在数据缓冲区哈希表 (dataBufHashTable) 中查找是否存在相同地址的缓冲区条目（命中检查）。

LRU 更新阶段：
如果命中，则将该缓冲区条目移动到 LRU 链表头部（表示最近被访问），以维护缓存替换的顺序。
*/
unsigned int CheckDataBufHit(unsigned int reqSlotTag)
{
	...
	while(bufEntry != DATA_BUF_NONE)
	{
		if(dataBufMapPtr->dataBuf[bufEntry].logicalSliceAddr == logicalSliceAddr)
		{
			...
			LRU 链表维护着缓冲区的“最近使用顺序”，头部是最新使用的条目，尾部是最久未使用的条目。
			...
			*/
			...
			情况	说明	操作
			有前驱、有后继	在链表中间	前驱→next = 后继；后继→prev = 前驱
			无后继、有前驱	在链表尾	前驱→next = NONE；更新 tailEntry
			有后继、无前驱	在链表头	后继→prev = NONE；更新 headEntry
			无前驱、无后继	唯一节点	head 和 tail 都置为 NONE
			*/
			...
			当前条目的 nextEntry = 原头结点

			原头结点的 prevEntry = 当前条目
